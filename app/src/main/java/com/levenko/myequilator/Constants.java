package com.levenko.myequilator;

/**
 * Created by Vilagra on 07.03.2017.
 */

@SuppressWarnings("DefaultFileTemplate")
public class Constants {
    public final static int REQUEST_CODE_RANGE = 1;
    public final static int REQUEST_CODE_CARD = 2;
    public final static int LOADER_ID = 1;
    public final static String POSITION_OF_ADAPTER="position_of_adapter";


    public final static String MY_LOG = "my_looogs";
    public final static String STRNGS_FROM_ADAPTER = "strings_from_adapter";
    public final static String STRNGS_FROM_STREET_ADAPTER = "strings_from_street_adapter";
    public final static String CURRENT_TAG = "curent_tag";
    public final static String EQUITY = "equity";
    public final static String INDEXES_DATA_WAS_CHOSEN = "indexes_data";
    public final static String STREET_ADAPTER = "street_adapter";
    public final static String POSITION_ADAPTER = "position_adapter";
    public static final String KIND_OF_ADAPTER = "kind_of_adapter" ;
    static final String WAS_CHOSEN= "was chosen";
    static final String NUMBER_OF_CARDS= "number cards";
    static final String IS_RESULT_DELIVERED= "is result delivered";

    public static final String INDEXES_DATA_WAS_CHOSEN_BY_POSITION_ADAPTER = "indexes_from_position_adapter";
    public static final String INDEXES_DATA_WAS_CHOSEN_BY_STREET_ADAPTER = "indexes_from_street_adapter";
    public static final String WINS = "wins";
    public static final String PARTIAL_POTS = "partial pots";
    public static final String TRAIL = "gameAmount";
    public static final String wasAdsDisabled = "was_disabled";
}
